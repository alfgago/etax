...
class Company extends Model {
    ...
    public static function checkCountAvailableBills() {

        $query = Company::query();

        $query->leftJoin('user_subscriptions_history', 'user_subscriptions_history.unique_no', '=', 'companies.plan_no');
        $query->leftJoin('subscription_plans', 'subscription_plans.id', '=', 'user_subscriptions_history.plan_id');
        $query->where('companies.id', currentCompany());

        $plan_details = $query->select('subscription_plans.no_of_bills')->first();

        $bill_count = \App\Bill::where(array('company_id' => currentCompany(), 'month' => date('m'), 'year' => date('Y')))->count();

        if ($plan_details) {

            if (is_null($plan_details->no_of_bills)) {//If bills are unlimited
                return 'unlimited';
            } else {
    ...
    public static function checkCountAvailableInvoices() {

        $query = \App\Company::query();
        ...
        $invoice_count = \App\Invoice::where(array('company_id' => currentCompany(), 'month' => date('m'), 'year' => date('Y')))->count();
