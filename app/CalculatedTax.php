...
class CalculatedTax extends Model
{
    ...
    public static function validarMes($date, $company = false)
    {
        ...
        try {
            ...
            $limiter = strpos($date, "-") === false ? "/" : "-";
            $generated_date = explode($limiter, $date);
    ...
    public function setDatosEmitidos($month, $year, $company)
    {
        ...
        InvoiceItem::with('invoice')
            ...
            ->where('year', $year)
            ->where('month', $month)
            ->chunk(2500, function ($invoiceItems) use (
                $year, $month, &$company, &$ivaData, &$ivaDevuelto, $filterTotales, $arrayActividades,
    ...
    public function setDatosSoportados($month, $year, $company, $query, $singleBill = false)
    {
        ...
        $query->chunk(2500, function ($billItems) use (
            $year, $month, &$company, &$ivaData, &$singleBill, &$basesConIvaDevuelto, $arrayActividades,
