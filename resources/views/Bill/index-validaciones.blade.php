            ...
            @if ( $bills->count() )
              @foreach ( $bills as $data )
                <tr>
                  ...
                  <td>
                    <a link="/facturas-recibidas/validar/{{ $data->id }}" titulo="Verificación Compra" class="btn btn-primary m-0 verificar_compra" style="color:#fff; font-size: 0.85em;" onclick="validarPopup(this);" data-toggle="modal" data-target="#modal_estandar">Validar</a>
                  </td>
                </tr>
              @endforeach
            @endif
