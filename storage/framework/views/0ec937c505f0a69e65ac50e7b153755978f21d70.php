  ...
  <div class="popup-container nuevo-proveedor-form form-row">
    ...
    <div class="form-group col-md-12">
      <div class="botones-agregar">
        <div onclick="agregarProveedorNuevo();" class="btn btn-dark m-1 ml-0">Confirmar proveedor</div>
        <div onclick="cerrarPopup('nuevo-proveedor-popup');" class="btn btn-danger m-1">Cancelar</div>
      </div>
    </div>
		...
		<script>
      	...
      	if( allow ){
              ...
              if(! $('#nuevo-proveedor-opt').length ) {
                  $("select#provider_id > option:nth-of-type(1)").after("<option id='nuevo-proveedor-opt' value='-1'> NUEVO - "+ cnombre +" </option>");
              }else{
                  $('#nuevo-proveedor-opt').text( "NUEVO - "+ cnombre );
              }
              $('select#provider_id').select2();
              $('select#provider_id').val('-1');
              $('select#provider_id').change();
...
<?php /**PATH /home/237808.cloudwaysapps.com/ducfpkkugc/public_html/resources/views/Bill/form-nuevo-proveedor.blade.php ENDPATH**/ ?>